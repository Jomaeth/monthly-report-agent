...
Agent skill：將項目嘅月度 Excel 資料包變成一份經驗證、可交管理層嘅月報 —— 然後停在人手批核關卡。AI 起草，人檢查，人簽名。

## Structure

```
monthly-report-agent/
├── SKILL.md                         [md]      agent 指令：Intent · Rules · Tools · Run · Gate
├── AGENTS.md                        [md]      Codex / agent 入口 → 指向 SKILL.md
├── workflows/
│   └── monthly_report_sop.md        [md]      人做法 SOP —— PM 平時點砌月報
├── tools/                           [python]  10 個 deterministic tool + 2 個 helper
...
│   ├── render_monthly_report.py               5 DRAFT
...
│   ├── send_report_email.py                   10 GATE
│   ├── _common.py                             helper：路徑 / .env
│   └── report_utils.py                        helper：RAG 排序 / 狀態正規化
├── config/report_sections.json      [config]  報告章節、品牌色、email 預設
├── brand_assets/                    [asset]   報告用 logo
├── tests/                           [python]  單元測試（pytest）
├── input/                           [input]   月度資料包 .xlsx —— 唔會 commit
└── outputs/monthly_report/<YYYY-MM>/ [output]  所有產出
```
...
| 類型 | 檔案 | 邊個寫 | 作用 |
|---|---|---|---|
| `.md` | `SKILL.md` | 識砌月報嘅人（PM / 商務） | 話俾 agent 聽：目標、規則、tool 次序、幾時停 |
| `.md` | `workflows/monthly_report_sop.md` | 同上 | 人手做法逐步寫低；SKILL.md 係佢嘅 agent 版 |
| `.py` | `tools/*.py` | 開發者 | 確定性運算：讀 Excel、驗證、計 KPI、出圖、出報告、把關 |
| `.json` | `config/report_sections.json` | PM / 商務 | 章節同品牌參數：改呢度，唔使改 code |

## Deterministic tools — 10 個（+2 helper）
...
| 5 | `render_monthly_report.py` | metrics + charts + config | `monthly_report.md / .html / .pdf` | 敘述式報告 |
...
| 10 | `send_report_email.py` | manifest + gate status | （拒絕 / 送出） | 有 blocker 就拒絕送出 |

## Output 點樣產生

```
input/<data pack>.xlsx
   │  run_monthly_report.py  ＝ load → validate → kpis → charts → render → email draft   (python · 確定性)
   ▼
outputs/monthly_report/<period>/
   ...
   ├── monthly_report.md/.html/.pdf   ← 敘述報告（render）
   └── email/                    ← 草稿 + manifest（email draft）
   │  generate_premium_html_report.py                                                  (python · 確定性)
   ▼
   ├── monthly_report_premium.html    ← 管理層睇嗰份
   │  visual_qa_report.py --simulate                                                   (python · 確定性)
   ▼
   ├── visual_qa_status.json / visual_qa_report.html
   │  agent 回報：行咗咩、幾多 blocker、RAG、報告路徑                                  (model · 摘要)
   ▼
人：清 blocker · 檢查 · 簽名                                                         (gate)
   │  send_report_email.py —— 有 blocker → 拒絕
```

Model 唔計任何數字，只跟 SKILL.md 次序 call tool 同回報結果。
...
```
跟住 SKILL.md 做：用 input 入面個資料包，出 2026-04 嘅月度報告，然後做 visual QA。
```
...
Python 3.10+ · pandas · openpyxl · matplotlib · reportlab · Pillow。標準執行唔使 API key；真實 vision QA 要 `.env` 入面 `GEMINI_API_KEY`。
